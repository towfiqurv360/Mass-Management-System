...
export default function BazaarPlannerPage() {
  ...
  // ==========================================
  // 🧮 ENTERPRISE SMART CALCULATOR STATES
  // ==========================================
  const [isCalcOpen, setIsCalcOpen] = useState(false);
  const [newCalcItemName, setNewCalcItemName] = useState("");
  const [newCalcItemPrice, setNewCalcItemPrice] = useState("");
  
  const defaultCalcItems = [
    ...
    { id: 6, name: "Fish", price: 350, qty: 0 },
    { id: 7, name: "Soyabean Oil", price: 170, qty: 0 },
    { id: 8, name: "Mixed Vegetables", price: 50, qty: 0 }
  ];
  
  const [calcItems, setCalcItems] = useState<any[]>(defaultCalcItems);

  useEffect(() => {
    setIsMounted(true);
    // 🚀 Load saved custom calculator items from local storage
    const savedCalcItems = localStorage.getItem("enterprise_bazaar_calc");
    if (savedCalcItems) {
      setCalcItems(JSON.parse(savedCalcItems));
    }
    fetchBazaarData();
  }, [selectedDate]);

  // 🚀 Save to local storage whenever calcItems change (so custom items are never lost)
  useEffect(() => {
    if (isMounted) {
      localStorage.setItem("enterprise_bazaar_calc", JSON.stringify(calcItems));
    }
  }, [calcItems, isMounted]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      await supabase.from("bazaar_planner").insert({
        item_name: userMessage, quantity: "Confidential Note", estimated_cost: 0,
        status: "private_msg", suggested_by: profileName, date: selectedDate
      });
  ...
  // 🚀 CALCULATOR LOGIC
  const handleAddNewCalcItem = () => {
    if (!newCalcItemName.trim() || !newCalcItemPrice) return toast.error("Provide item name and price.");
    const newItem = {
      id: Date.now(),
      name: newCalcItemName,
      price: Number(newCalcItemPrice),
      qty: 0
    };
    setCalcItems([...calcItems, newItem]);
    setNewCalcItemName("");
    setNewCalcItemPrice("");
    toast.success(`${newCalcItemName} added to calculator!`);
  };
    ...
    <div className="relative min-h-[80vh] w-full max-w-7xl mx-auto space-y-6 animate-fade-in z-0 pb-10 px-4 md:px-0 print:p-0 print:bg-white text-slate-900 dark:text-white">
      ...
      <div className="bg-white/60 dark:bg-[#0F172A]/70 backdrop-blur-3xl p-5 md:p-6 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5 print:hidden transition-all duration-300 hover:shadow-indigo-500/5">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto items-end sm:items-center">
          ...
          <button onClick={handleDownloadList} className="px-5 py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-md active:scale-95 flex items-center justify-center gap-2 cursor-pointer border border-transparent dark:border-slate-200 hover:bg-slate-800 dark:hover:bg-slate-100">
            📥 Print Ledger
          </button>
          <input 
            type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} 
            className="w-full sm:w-auto px-5 py-3.5 text-sm font-black bg-white dark:bg-slate-800 border-2 border-indigo-50 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 shadow-sm cursor-pointer appearance-none transition-all text-slate-800 dark:text-white"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-1 space-y-6 sm:space-y-8 print:hidden">
          ...
          {/* Admin Grocery Adder OR User Private Note */}
          {actualRole === "admin" ? (
            <div className="bg-white/70 dark:bg-[#0F172A]/70 backdrop-blur-3xl p-6 md:p-8 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-2xl relative overflow-hidden group">
              ...
              <form onSubmit={handleAddItem} className="space-y-4">
                ...
                <button type="submit" disabled={actionLoading} className="cursor-pointer w-full py-4 mt-2 bg-gradient-to-r from-teal-500 to-emerald-600 text-white font-black uppercase text-xs tracking-widest rounded-xl shadow-lg hover:shadow-emerald-500/25 hover:-translate-y-0.5 active:scale-95 transition-all">Add to Ledger</button>
              </form>
            </div>
          ) : (
            <div className="bg-gradient-to-br from-rose-50 to-rose-100 dark:from-rose-950/40 dark:to-slate-900 backdrop-blur-3xl p-6 md:p-8 rounded-[2rem] border border-rose-200 dark:border-rose-900/50 shadow-2xl relative overflow-hidden group">
              <h3 className="text-xs font-black text-rose-600 dark:text-rose-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                <span>🔒</span> Confidential Note
              </h3>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mb-5 leading-relaxed">
                Dietary restrictions or requests? Drop a message here. <strong className="text-rose-500">Private to Manager.</strong>
              </p>
              <form onSubmit={handleSendMessage} className="space-y-4">
                <textarea 
                  placeholder="e.g. 'Manager, I don't eat beef. Manage chicken for me.'" 
                  required value={userMessage} onChange={e => setUserMessage(e.target.value)} 
                  className="w-full px-4 py-3.5 text-sm font-semibold bg-white dark:bg-slate-900 border-2 border-rose-100 dark:border-rose-900/50 rounded-xl outline-none focus:border-rose-400 transition-all text-slate-800 dark:text-white min-h-[120px] resize-none shadow-inner"
                />
                <button type="submit" disabled={actionLoading} className="cursor-pointer w-full py-4 bg-rose-500 hover:bg-rose-600 text-white font-black uppercase text-xs tracking-widest rounded-xl shadow-lg hover:shadow-rose-500/25 hover:-translate-y-0.5 active:scale-95 transition-all">
                  Send Securely
                </button>
              </form>
                ...
                <div className="mt-6 pt-4 border-t border-rose-200 dark:border-rose-900/50">
                  ...
                  <div className="space-y-2">
                    {mySentMessages.map(msg => (
                      <div key={msg.id} className="bg-white/50 dark:bg-slate-900/50 p-3 rounded-lg text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-100 dark:border-slate-800 shadow-sm">
                        {msg.item_name}
                      </div>
                    ))}
                  </div>
          ...
          {/* MANAGER EXCLUSIVE: Assign Buyers & Post Image */}
          {actualRole === "admin" && (
              ...
              <form onSubmit={handleSaveAssignment} className="space-y-5 relative z-10">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" placeholder="Name" required value={p1} onChange={e => setP1(e.target.value)} className="w-full px-4 py-3 text-xs font-bold bg-slate-800 border-2 border-slate-700 rounded-xl outline-none focus:border-indigo-500 focus:bg-slate-900 transition-all text-white shadow-inner" />
                  </div>
                  <div>
                    <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">Buyer 2</label>
                    <input type="text" placeholder="Name" required value={p2} onChange={e => setP2(e.target.value)} className="w-full px-4 py-3 text-xs font-bold bg-slate-800 border-2 border-slate-700 rounded-xl outline-none focus:border-indigo-500 focus:bg-slate-900 transition-all text-white shadow-inner" />
                  </div>
                </div>
          ...
          {/* Live Duty Card */}
          {assignment.person_1 && (
            <div className="bg-gradient-to-tr from-emerald-500 to-teal-600 p-6 md:p-8 rounded-[2rem] shadow-xl shadow-emerald-500/20 text-white relative overflow-hidden border border-emerald-400/50 group">
        ...
        <div className="lg:col-span-2 space-y-6 sm:space-y-8 print:col-span-3">
          ...
          {/* ADMIN INBOX */}
          {actualRole === "admin" && adminInbox.length > 0 && (
            <div className="bg-white/60 dark:bg-[#0F172A]/70 backdrop-blur-3xl rounded-[2rem] border border-rose-200 dark:border-rose-900/30 shadow-xl overflow-hidden print:hidden transition-all duration-300 hover:shadow-rose-500/5">
              <div className="p-5 md:p-6 border-b-2 border-rose-100 dark:border-rose-900/50 bg-rose-50/50 dark:bg-rose-500/5 flex items-center gap-3">
                <div className="p-2 bg-rose-100 dark:bg-rose-500/20 rounded-xl text-rose-600 dark:text-rose-400 shadow-inner">🔒</div>
                <div>
                  <h3 className="text-sm font-black text-rose-800 dark:text-rose-300 uppercase tracking-widest">Resident Confidential Notes ({adminInbox.length})</h3>
          ...
          <div className="bg-white/70 dark:bg-[#0F172A]/70 backdrop-blur-3xl rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-2xl overflow-hidden print:border-none print:shadow-none print:bg-transparent transition-all duration-300 hover:shadow-indigo-500/5">
            <div className="p-6 md:p-8 border-b-2 border-slate-100 dark:border-slate-800 bg-white/40 dark:bg-slate-800/20 flex justify-between items-center gap-4 print:hidden">
              ...
              <div className="text-right">
                 ...
                 <span className="text-xl md:text-2xl font-black text-slate-800 dark:text-white tracking-tighter">৳ {totalEstCost.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="hidden print:block border-t-2 border-slate-900 p-4 text-right font-black text-lg">
              Total Target Budget: ৳ {totalEstCost.toLocaleString()}
            </div>
          </div>

          {/* Handwritten Image Displayer */}
          {assignment.image_url && (
            <div className="bg-white/50 dark:bg-[#0F172A]/50 backdrop-blur-3xl rounded-[2rem] border border-white/50 dark:border-slate-700/50 p-6 space-y-4 print:hidden group shadow-xl hover:shadow-indigo-500/5 transition-all">
      ...
      {/* ========================================================= */}
      {/* 🚀 ENTERPRISE FEATURE: SMART CALCULATOR MODAL             */}
      {/* ========================================================= */}
      {isCalcOpen && actualRole === "admin" && (
        ...
        >
          <div className="bg-white dark:bg-[#0F172A] w-full max-w-4xl rounded-[2.5rem] shadow-2xl border-2 border-slate-200/50 dark:border-slate-700/80 overflow-hidden relative transform transition-all animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-6 md:p-8 border-b-2 border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                  ...
                  Enterprise Market Calculator
                </h3>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">Custom items are auto-saved in your browser for next time.</p>
              </div>
            ...
            <div className="p-6 md:p-8 flex-1 overflow-y-auto custom-scrollbar bg-slate-50/30 dark:bg-transparent">
              ...
              {/* Dynamic Adder Form */}
              <div className="bg-indigo-50 dark:bg-indigo-500/10 border-2 border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-4 md:p-5 mb-6 flex flex-col md:flex-row gap-4 items-end shadow-inner">
                <div className="w-full md:flex-1">
                  <label className="block text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1.5">New Item Name</label>
                  <input type="text" placeholder="e.g. Tomato, Rui Fish" value={newCalcItemName} onChange={e => setNewCalcItemName(e.target.value)} className="w-full px-4 py-3 text-sm font-bold bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 transition-all text-slate-800 dark:text-white" />
                </div>
                <div className="w-full md:w-48">
                  <label className="block text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1.5">Price Per Unit (৳)</label>
                  <input type="number" placeholder="e.g. 50" value={newCalcItemPrice} onChange={e => setNewCalcItemPrice(e.target.value)} className="w-full px-4 py-3 text-sm font-bold bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-indigo-500 transition-all text-slate-800 dark:text-white" />
                </div>
                <button onClick={handleAddNewCalcItem} className="w-full md:w-auto px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-md active:scale-95 cursor-pointer flex items-center justify-center gap-2">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 4v16m8-8H4" /></svg> Add Item
                </button>
              </div>

              {/* Table Header */}
              <div className="grid grid-cols-12 gap-4 text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3 px-2">
                <div className="col-span-5 md:col-span-4">Item Name</div>
                <div className="col-span-3 md:col-span-3 text-center">Unit Price (৳)</div>
              ...
              <div className="space-y-3">
                {calcItems.map((item, index) => (
                  <div key={item.id} className="grid grid-cols-12 gap-3 md:gap-4 items-center bg-white dark:bg-slate-800/80 p-3 md:p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:shadow-md hover:border-indigo-500/30 group">
                    
                    <div className="col-span-5 md:col-span-4 font-bold text-xs md:text-sm text-slate-800 dark:text-white tracking-tight break-words">{item.name}</div>
                    
                    {/* Editable Price */}
                    <div className="col-span-3 md:col-span-3 relative flex items-center justify-center">
                      <span className="absolute left-2 text-slate-400 font-bold text-[10px]">৳</span>
                      <input 
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl py-2 pl-5 md:pl-6 pr-1 md:pr-2 text-xs font-black text-center text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                      />
                    </div>
                    
                    {/* Premium Pro Quantity Control (Input + Buttons) */}
                    <div className="col-span-4 md:col-span-4 flex items-center justify-center gap-1 bg-slate-50 dark:bg-slate-900 p-1 md:p-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
                      <button onClick={() => { const newItems = [...calcItems]; newItems[index].qty = Math.max(0, item.qty - 0.5); setCalcItems(newItems); }} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center rounded-lg bg-white dark:bg-slate-800 text-slate-500 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/20 font-black cursor-pointer shadow-sm active:scale-90 transition-all">-</button>
                      
                      <input 
                        type="number" step="0.1" value={item.qty} 
                        onChange={(e) => {
                          const newItems = [...calcItems];
                          newItems[index].qty = Number(e.target.value);
                          setCalcItems(newItems);
                        }}
                        className="w-10 md:w-14 text-center bg-transparent text-xs md:text-sm font-black text-indigo-600 dark:text-indigo-400 outline-none"
                      />
                    ...
                    <div className="hidden md:flex col-span-1 justify-end">
                       <button onClick={() => removeCalcItem(item.id)} className="w-8 h-8 rounded-full bg-rose-50 dark:bg-rose-500/10 text-rose-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-500 hover:text-white cursor-pointer" title="Remove Item">
            ...
            <div className="bg-slate-900 dark:bg-black p-6 md:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shrink-0 border-t-4 border-indigo-500">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Total Estimated Cost</p>
