...
export default function MonthlyBillingPage() {
  ...
  // =========================================================
  // 🚀 ENTERPRISE: DYNAMIC RATE CARD & GLOBAL BILLING STATES
  // =========================================================
  const defaultRates = {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Load saved rates from local storage if manager previously updated them
  ...
  const fetchBillingData = async () => {
    ...
    try {
      ...
      const billingArray = (profilesData || []).map(p => {
        ...
        return { 
          ...p, totalMeals: myTotalMeals, mealCost: myMealCost, mealDeposit: myMealDeposit, mealDueOrRefund, fixedMessBill 
        };
      });
  ...
  const handleArchiveMonth = async () => {
    ...
    if (!window.confirm(`⚠️ WARNING: You are about to permanently CLOSE the month of ${selectedMonth}.\n\n1. This month's bazaar/meal history will be archived.\n2. Members' fixed bills will reset to 0.\n3. Balances will be finalized.\n\nProceed?`)) return;
    ...
    try {
      ...
      await supabase.from("profiles").update({
        current_month_rent: 0, current_month_maid: 0, current_month_wifi: 0, current_month_electricity: 0
      }).not("id", "is", null);

      toast.success("Month closed successfully!", { id: toastId });
      setIsMonthClosed(true);
      fetchBillingData();
    } catch (error) {
  ...
  // =========================================================
  // 🚀 ENTERPRISE: THE SMART GLOBAL GENERATOR
  // =========================================================
  const handleRateChange = (category: string, field: string, value: string) => {
    ...
    localStorage.setItem("mess_pro_rate_card", JSON.stringify(updatedRates)); // Auto-saves changes!
  };

  const handleGlobalGenerate = async () => {
    if (userRole !== "super_admin") return toast.error("Unauthorized.");
    if (isMonthClosed) return toast.error("Cannot bill a closed month.");

    if (!window.confirm("⚡ Master Trigger: This will instantly assign Rent, Maid, WiFi bills to ALL members based on their Room Category, and add the given Electricity bill. Proceed?")) return;

    setActionLoading(true);
    const toastId = toast.loading("Executing Global Smart Billing Engine...");
    ...
    try {
      ...
      const promises = (profiles || []).map(p => {
        // 1. Determine rates based on this specific user's category
      ...
      toast.success("Global Bills successfully generated for everyone!", { id: toastId });
      setGlobalElectricity(""); // Clear electricity input after success
      fetchBillingData();
    } catch (error) {
      toast.error("Global Billing failed.", { id: toastId });
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
    <div className="relative min-h-[80vh] w-full max-w-7xl mx-auto space-y-6 animate-fade-in z-0 pb-10 px-4 md:px-0">
      ...
      {/* Background Glows */}
      <div className="fixed top-20 left-10 w-96 h-96 bg-amber-500/10 rounded-full filter blur-[100px] pointer-events-none -z-10 print:hidden transition-transform duration-500 hover:scale-110"></div>
      <div className="fixed bottom-10 right-10 w-96 h-96 bg-rose-500/10 rounded-full filter blur-[100px] pointer-events-none -z-10 print:hidden transition-transform duration-500 hover:scale-110"></div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            {isMonthClosed ? (
              ...
              <span className="px-3 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 rounded-lg text-[10px] font-black uppercase tracking-widest border border-emerald-200 dark:border-emerald-500/30 flex items-center gap-1.5 animate-pulse shadow-inner">
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> 🟢 Live Data
              </span>
            )}
          </div>
          <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-[11px] font-bold mt-1.5 uppercase tracking-widest flex items-center gap-2">Financial Settlement & History</p>
        </div>
        
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto items-end sm:items-center">
          <button onClick={() => window.print()} className="px-5 py-3 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-black uppercase tracking-widest rounded-xl transition-all hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm active:scale-95 flex items-center justify-center gap-2">
             🖨️ Print Ledger
          </button>
          
          {userRole === "super_admin" && !isMonthClosed && (
            <button onClick={handleArchiveMonth} disabled={actionLoading} className="px-5 py-3.5 bg-gradient-to-r from-rose-600 to-pink-600 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-xl hover:shadow-rose-500/30 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-70 disabled:hover:scale-100 cursor-pointer">
               🔒 Close Month
            </button>
          )}

          <input type="month" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="w-full sm:w-auto px-5 py-3.5 text-sm font-black bg-slate-100 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 shadow-inner cursor-pointer appearance-none transition-all" />
        </div>
      </div>

      {/* ======================================================= */}
      {/* 🚀 SMART RATE CARD CONFIGURATOR (Super Admin Only)      */}
      {/* ======================================================= */}
      {userRole === "super_admin" && (
        <div className={`bg-gradient-to-br from-slate-900 via-[#0F172A] to-slate-900 p-6 md:p-8 rounded-[2rem] border border-slate-700 shadow-2xl print:hidden text-white transition-all duration-300 relative overflow-hidden group ${isMonthClosed ? 'opacity-60 pointer-events-none grayscale hover:shadow-none' : 'hover:shadow-amber-500/10 hover:-translate-y-1 hover:border-slate-600'}`}>
          <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/5 rounded-full blur-3xl pointer-events-none transition-transform duration-700 group-hover:scale-150"></div>
          
          <div className="relative z-10 flex flex-col lg:flex-row gap-8">
            
            {/* LEFT SIDE: Dynamic Rate Card Table */}
            <div className="flex-1 w-full border-b lg:border-b-0 lg:border-r border-slate-700 pb-6 lg:pb-0 lg:pr-8">
              <h3 className="text-sm font-black text-amber-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                <span className="p-1.5 bg-amber-500/10 rounded-lg"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" /></svg></span>
                Default Category Rate Card
              </h3>
              <p className="text-[10px] text-slate-400 font-bold mb-5 tracking-wider">Manager can customize base rates here. Values are auto-saved for the future.</p>
              
              {/* Rate Header */}
              <div className="grid grid-cols-4 gap-3 text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2 pl-2">
                <div className="col-span-1">Room Type</div>
                <div className="text-center">Seat Rent</div>
                <div className="text-center">Maid Bill</div>
                <div className="text-center">WiFi Bill</div>
              </div>

              {/* Rate Rows */}
              <div className="space-y-2">
                {Object.keys(categoryRates).map((cat) => (
                  <div key={cat} className="grid grid-cols-4 gap-3 items-center bg-slate-800/40 p-2 rounded-xl border border-slate-700/50 hover:bg-slate-800/80 transition-colors">
                    <div className="col-span-1 font-bold text-[11px] sm:text-xs text-white pl-2">{cat}</div>
                    <div className="relative">
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-[10px]">৳</span>
                      <input type="number" value={categoryRates[cat].rent} onChange={(e) => handleRateChange(cat, 'rent', e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2 pl-6 pr-2 text-[11px] sm:text-xs font-bold text-center text-white outline-none focus:border-amber-500 transition-all" />
                    </div>
                    <div className="relative">
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-[10px]">৳</span>
                      <input type="number" value={categoryRates[cat].maid} onChange={(e) => handleRateChange(cat, 'maid', e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2 pl-6 pr-2 text-[11px] sm:text-xs font-bold text-center text-white outline-none focus:border-amber-500 transition-all" />
                    </div>
                    <div className="relative">
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-[10px]">৳</span>
                      <input type="number" value={categoryRates[cat].wifi} onChange={(e) => handleRateChange(cat, 'wifi', e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2 pl-6 pr-2 text-[11px] sm:text-xs font-bold text-center text-white outline-none focus:border-amber-500 transition-all" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* RIGHT SIDE: Electricity & Trigger */}
            <div className="flex-1 w-full flex flex-col justify-center">
              
              <div className="bg-amber-500/10 border border-amber-500/20 p-5 md:p-6 rounded-2xl mb-5">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h4 className="text-amber-400 font-black text-xs md:text-sm tracking-wide uppercase">Monthly Electricity Bill</h4>
                ...
                <div className="relative">
                  ...
                  <input 
                    type="number" 
                    value={globalElectricity} 
                    onChange={(e) => setGlobalElectricity(e.target.value)} 
                    placeholder="Total amount per seat" 
                    className="w-full bg-slate-900/80 border-2 border-amber-500/50 rounded-xl py-3.5 pl-10 pr-4 text-sm font-black text-amber-300 outline-none focus:border-amber-400 focus:bg-slate-900 transition-all placeholder:text-amber-500/30"
                  />
                </div>
              </div>
              ...
              <button 
                ...
                disabled={actionLoading || isMonthClosed} 
                className="cursor-pointer w-full py-4.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-900 font-black uppercase tracking-widest text-xs rounded-xl transition-all shadow-xl shadow-amber-500/20 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                Apply Bills To All Members
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 print:hidden">
        <div className="bg-white/60 dark:bg-[#0F172A]/70 backdrop-blur-3xl p-6 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-xl flex justify-between items-center group hover:-translate-y-1 transition-all hover:shadow-amber-500/5">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1">Total Month Meals</p>
          ...
          <div className="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform shadow-md border-2 border-white dark:border-slate-700">🍽️</div>
        </div>
        <div className="bg-white/60 dark:bg-[#0F172A]/70 backdrop-blur-3xl p-6 rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-xl flex justify-between items-center group hover:-translate-y-1 transition-all hover:shadow-emerald-500/5">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1">Total Bazaar Cost</p>
          ...
          <div className="w-14 h-14 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform shadow-md border-2 border-white dark:border-slate-700">🛒</div>
        </div>
        <div className="bg-gradient-to-br from-amber-500 to-rose-600 p-6 rounded-[2rem] shadow-xl shadow-rose-500/10 text-white flex justify-between items-center relative overflow-hidden group hover:-translate-y-1 transition-all hover:shadow-rose-500/20">
          ...
          <div className="z-10">
            <p className="text-[10px] font-black uppercase tracking-widest opacity-90 mb-1">Final Meal Rate</p>
      ...
      {/* 🧾 COMPACT LIST VIEW 🧾 */}
      <div className="bg-white/60 dark:bg-[#0F172A]/70 backdrop-blur-3xl rounded-[2rem] border border-white/50 dark:border-slate-700/50 shadow-2xl overflow-hidden hover:shadow-amber-500/5 transition-all duration-300">
        <div className="p-6 md:p-8 border-b border-slate-200/50 dark:border-slate-700/50 bg-white/40 dark:bg-slate-800/20 print:hidden flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Resident Directory & Ledger</h3>
            <p className="text-[10px] text-slate-500 font-bold mt-1 uppercase">Click any row for detailed month settlement</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest bg-slate-50/80 dark:bg-slate-800/40 border-b border-slate-200/50 dark:border-slate-700/50">
                ...
                <th className="px-6 py-5">Meal Cost</th>
            ...
            <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
              {loading ? (
                <tr><td colSpan={4} className="text-center py-20 text-xs font-black text-slate-400 uppercase tracking-widest animate-pulse">Syncing Master Ledger Data...</td></tr>
              ) : membersBilling.length === 0 ? (
                <tr><td colSpan={4} className="text-center py-20 text-sm text-slate-400 font-bold">No billing profiles found for this month.</td></tr>
              ) : membersBilling.map((member) => (
                <tr 
                  key={member.id} 
                  onClick={() => { setSelectedModalUser(member); setIsEditingIndividual(false); }}
                  className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-all cursor-pointer group active:scale-[0.99] relative"
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-bold text-sm text-slate-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors tracking-tight">{member.full_name}</div>
                        <div className="flex items-center gap-2 mt-1.5">
                          ...
                          <span className="text-[9px] font-black text-slate-400/90 uppercase tracking-wider">{member.room_category}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-black text-sm text-slate-800 dark:text-slate-200">৳ {member.mealCost.toFixed(2)}</div>
                    <div className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-wider">{member.totalMeals.toFixed(1)} Portions</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="font-black text-sm text-slate-800 dark:text-slate-200">৳ {member.fixedMessBill.toLocaleString()}</div>
                    <div className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-wider">Fixed Dues</div>
                  </td>
      ...
      {selectedModalUser && (
        ...
        >
          <div className="bg-white dark:bg-[#0F172A] w-full max-w-4xl rounded-[2.5rem] shadow-2xl border-2 border-slate-200/50 dark:border-slate-700/80 overflow-hidden relative transform transition-all animate-in zoom-in-95 duration-200">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2.5 animate-fade-in delay-300">
                    ...
                    <span className="px-2.5 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] font-bold text-slate-500 uppercase">{selectedModalUser.room_category}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6 animate-fade-in delay-200">
                ...
                <div className="space-y-3.5 bg-slate-50 dark:bg-slate-900/50 p-5 rounded-2xl border-2 border-slate-100 dark:border-slate-800 shadow-inner">
                  ...
                  <div className="flex justify-between items-center text-sm font-semibold text-slate-600 dark:text-slate-400 pb-3 border-b border-slate-100 dark:border-slate-800">
                    <span className="tracking-tight">Calculated Meal Cost</span>
                ...
                <div className={`mt-5 p-6 rounded-2xl border-2 flex justify-between items-center transition-all hover:scale-[1.01] ${selectedModalUser.mealDueOrRefund >= 0 ? 'bg-emerald-50 border-emerald-200 dark:bg-emerald-500/10 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 hover:shadow-emerald-500/5' : 'bg-rose-50 border-rose-200 dark:bg-rose-500/10 dark:border-rose-500/20 text-rose-700 dark:text-rose-400 hover:shadow-rose-500/5'}`}>
                  <div>
                    <p className="text-[10px] md:text-[11px] font-black uppercase tracking-widest">{selectedModalUser.mealDueOrRefund >= 0 ? 'Meal Advance / Refundable' : 'Meal Due Amount'}</p>
              ...
              <div className="space-y-6 border-t md:border-t-0 md:border-l-2 border-slate-100 dark:border-slate-800 pt-6 md:pt-0 md:pl-10 animate-fade-in delay-300">
                ...
                {isEditingIndividual ? (
                  ...
                  <div className="space-y-3.5 bg-slate-50 dark:bg-slate-900/50 p-5 md:p-6 rounded-2xl border-2 border-slate-100 dark:border-slate-800 shadow-inner">
                    <div className="flex justify-between text-sm font-semibold text-slate-600 dark:text-slate-400"><span>Seat Rent</span> <span className="text-slate-900 dark:text-white font-bold">৳ {selectedModalUser.current_month_rent || 0}</span></div>
                    <div className="flex justify-between text-sm font-semibold text-slate-600 dark:text-slate-400"><span>Maid/Cook</span> <span className="text-slate-900 dark:text-white font-bold">৳ {selectedModalUser.current_month_maid || 0}</span></div>
                    <div className="flex justify-between text-sm font-semibold text-slate-600 dark:text-slate-400"><span>WiFi Bill</span> <span className="text-slate-900 dark:text-white font-bold">৳ {selectedModalUser.current_month_wifi || 0}</span></div>
                    <div className="flex justify-between text-sm font-semibold text-slate-600 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-3"><span>Electricity</span> <span className="text-slate-900 dark:text-white font-bold">৳ {selectedModalUser.current_month_electricity || 0}</span></div>
                    ...
                    <div className="mt-5 p-6 rounded-2xl border-2 bg-amber-50 border-amber-200 dark:bg-amber-500/10 dark:border-amber-500/20 text-amber-700 dark:text-amber-400 transition-all hover:scale-[1.01] hover:shadow-amber-500/5">
                      <div>
                        <p className="text-[10px] md:text-[11px] font-black uppercase tracking-widest">Total Mess Rent (Payable)</p>
